...
namespace effil
{
	...
	namespace
	{
		...
		std::string luaError(int errCode)
		{
			...
			{
				case LUA_ERRSYNTAX:
					return "Invalid syntax (LUA_ERRSYNTAX)";
				case LUA_ERRMEM:
					return "Memory allocation error (LUA_ERRMEM)";
				case LUA_ERRRUN:
					return "Execution error (LUA_ERRRUN)";
				case LUA_ERRGCMM:
					return "Error in __gc method (LUA_ERRGCMM)";
				case LUA_ERRERR:
					return "Recursive error (LUA_ERRERR)";
				default:
					return "Unknown (" + std::to_string(errCode) + ")";
			}
		}
	...
	std::chrono::milliseconds fromLuaTime(int duration, const sol::optional<std::string>& period)
	{
		...
		if (metric == "ms")
			...
			throw sol::error("invalid time metric: " + metric);
	}
	...
	milliseconds Timer::left()
	{
		...
		return timeout_ > diff ? duration_cast<milliseconds>((timeout_ - diff)) : milliseconds(0);
	}
