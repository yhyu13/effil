...
namespace effil
{
	...
	namespace
	{
		...
		public:
			template <typename SolType>
			FunctionHolder(const SolType& luaObject)
				: GCObjectHolder<Function>(luaObject) {}
	...
	StoredObject createStoredObject(lua_Number value)
	{
		return std::make_unique<PrimitiveHolder<lua_Number>>(value);
	}

	StoredObject createStoredObject(lua_Integer value)
	{
		return std::make_unique<PrimitiveHolder<lua_Integer>>(value);
	}
	...
	sol::optional<LUA_INDEX_TYPE> storedObjectToIndexType(const StoredObject& sobj)
	{
		return getPrimitiveHolderData<LUA_INDEX_TYPE>(sobj);
	}
